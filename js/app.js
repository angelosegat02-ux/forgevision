...
function openStudio(path){overlay.classList.add('active');overlay.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';if(path)data.path=path;if(quickIdea.value&&!data.description)data.description=quickIdea.value;if(window.ForgeCore){ForgeCore.startProject(data)}renderStep()}
...
function bindDynamic(){stepBox.querySelectorAll('[data-key]').forEach(el=>el.addEventListener('click',()=>{data[el.dataset.key]=el.dataset.val;if(window.ForgeCore){ForgeCore.updateStudio(data)}renderStep()}));const desc=document.getElementById('descInput');if(desc)desc.addEventListener('input',e=>{data.description=e.target.value;if(window.ForgeCore){ForgeCore.updateStudio(data)}});const help=document.getElementById('helpIdea');if(help)help.addEventListener('click',()=>{data.description=(desc.value?desc.value+'\n\n':'')+'Mi aiuti a trasformare questa idea in un progetto chiaro, definendo uso, dimensioni, stile e dettagli funzionali.';renderStep()});const minus=document.getElementById('minusQty');const plus=document.getElementById('plusQty');if(minus)minus.onclick=()=>{data.quantity=Math.max(1,data.quantity-1);if(window.ForgeCore){ForgeCore.updateStudio(data)}renderStep()};if(plus)plus.onclick=()=>{data.quantity++;if(window.ForgeCore){ForgeCore.updateStudio(data)}renderStep()};const send=document.getElementById('sendRequest');if(send)send.onclick=()=>{if(window.ForgeCore){ForgeCore.updateStudio(data);console.log('ForgeCore project',ForgeCore.getProject())}alert('Sprint 1 attivo: il progetto è stato salvato nella memoria ForgeCore. Nel prossimo sprint collegheremo il Designer intelligente.')}} 
